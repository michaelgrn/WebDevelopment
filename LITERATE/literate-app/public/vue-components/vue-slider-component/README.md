...
NOTE FROM LITERATE DEVELOPER
* Cloned this copy of the vue2-slider component into our Express project's public directory because the library we are using to render vue components using Express, express-vue, renders vue components on 
 the server side.  vue2-slider requires access to client elements, such as Document, which requires the component to be referenced on the client-side.  This component is copied here purely for that reason.  If another solution for this problem is discovered, this copied component can be removed.  The reference to the component will just have to change to the node_modules (scripts) path.  This copy of the component is version 2.6.2.
